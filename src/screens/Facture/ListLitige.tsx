...
const ListLitige = () => {
    ...
    const { data, isError, isLoading } = useQuery({
        queryKey: ["litige", params.factureId],
                    ...
                    data.$values.map((litige: any) => (
                        <View key={litige.litigeId} style={[styles.card]}>
                            <View style={styles.header, [layout.row]}>
                                ...
                                >
                                    <Text style={styles.statusButtonText}>
                                        {litige.statut === 0 ? 'Approved' : litige.statut === 2 ? 'Pending' : 'Rejected'}
                                    </Text>
                                </TouchableOpacity>
                            </View>
                            ...
                            <View style={styles.infoRow}>
                                ...
                                <Text style={styles.value}>{formatDate(litige.dateLitige)}</Text>
                            </View>
                            <View style={styles.infoRow}>
                                <Text style={styles.label}>Date Echeance :</Text>
                                <Text style={styles.value}>{formatDate(litige.dateEcheanceLitige)}</Text>
                            </View>
                            <View style={styles.infoRow}>
