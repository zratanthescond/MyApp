...
const ListProrogation = () => {
    ...
    const { data, isError, isLoading } = useQuery({
        queryKey: ["prorogation", params.factureId],
    ...
    if (isError) {
        return <Text style={styles.errorText}> Erreur lors du chargement des Prorogations.</Text>;
    }
                        ...
                        <View key={prorogation.prorogationId} style={[styles.card]} >
                            <View style={styles.header, [layout.row]} >
                                ...
                                <TouchableOpacity
                                    ...
                                    style={[styles.statusButton, getButtonStyle(prorogation.statut)]}
                                >
                                    <Text style={styles.statusButtonText} >
                                        {prorogation.statut === 0 ? 'Approved' : prorogation.statut === 2 ? 'Pending' : 'Rejected'}
                                    </Text>
                                </TouchableOpacity>
                            </View>
                            ...
                            < View style={styles.infoRow} >
                                ...
                                < Text style={styles.value} > {formatDate(prorogation.dateEcheanceApresProrogation)}</Text>
                            </View>
                            < View style={styles.infoRow} >
                                <Text style={styles.label}>  Echeance: </Text>
                                < Text style={styles.value} > {formatDate(prorogation.echeance)} </Text>
                            </View>


                            < View style={styles.infoRow} >
                                <Text style={styles.label}> Motif : </Text>
                                < Text style={styles.value} > {prorogation.motifProrogation} </Text>
                            </View>
