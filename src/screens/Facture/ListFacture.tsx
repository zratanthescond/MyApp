...
export default function ListFacture() {
    ...
    const { data, isError, isLoading } = useQuery({
        queryKey: ["facture", bordereauId],
        queryFn: () => getFactureByBordereau({ newBordereauId: bordereauId, contractId }),
    });
    ...
    const handleNavigation = (factureId: number, refFacture: string, path: string) => {
        if (path === "litige") {
    ...
    return (
        <SafeScreen>
            <View style={[{ height: "100%", width: "100%" }, gutters.paddingHorizontal_12]}>
                <View
                    style={[
                        layout.row,
                        layout.itemsCenter,
