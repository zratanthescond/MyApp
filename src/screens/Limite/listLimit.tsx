...
import listLimit from '@/services/Limit/listLimit';
...
const ListLimit = () => {
    ...
    const { data, isError, isLoading } = useQuery({
        queryKey: ["limite", contractId],
        queryFn: () => listLimit(contractId),
    });
    ...
    if (isError) {
        return <Text style={styles.errorText}>Erreur lors du chargement des Limites.</Text>;
    }
    ...
    return (
        <View style={styles.mainContainer}>
            <View style={styles.header}>
                <Text style={styles.title}>Limites Détails</Text>
            </View>

            <ScrollView contentContainerStyle={styles.container}>
                {data && data.$values.length > 0 ? (
                    data.$values.map((limite: any) => (
                        <View key={limite.id} style={[styles.card]}>
                            ...
                            <View style={styles.infoRow}>
                                ...
                                <Text style={styles.value}>{formatDate(limite.dateDemande)}</Text>
                            </View>
                            <View style={styles.infoRow}>
                                <Text style={styles.label}>Date Dernier Demande:</Text>
                                <Text style={styles.value}>{formatDate(limite.dateDerniereDemande)}</Text>
                            </View>
                            <View style={styles.infoRow}>
                                <Text style={styles.label}>Date Limit:</Text>
                                <Text style={styles.value}>{formatDate(limite.dateDemande)}</Text>
                            </View>

                            <View style={styles.infoRow}>
                                <Text style={styles.label}>Limit Financement:</Text>
                                <Text style={styles.value}>{limite.limiteFinancement}</Text>
                            </View>
                            <View style={styles.infoRow}>
                                <Text style={styles.label}>Délai Financement:</Text>
                                <Text style={styles.value}>{limite.delaiDemande}</Text>
                            </View>
