...
const ListFinancement = () => {
    ...
    const { data, isError, isLoading } = useQuery({
        queryKey: ["financement", contractId],
                ...
                {filteredFinancements && filteredFinancements.length > 0 ? (
                    filteredFinancements.map((financement) => (
                        <View
                            ...
                            style={[styles.card, { backgroundColor: getBackgroundColor(financement.statutFinancement) }]}>
                            <View style={styles.header}>
                                <Text style={styles.title}>Financement </Text>
                            </View>
                            <Text style={styles.price}>{financement.montantFinancement} TND</Text>
                            ...
                            <View style={styles.infoRow}>
                                ...
                                <Text style={styles.value}>{financement.typeDeFinancement}</Text>
                            </View>
                            <View style={styles.infoRow}>
                                <Text style={styles.label}>Date :</Text>
                                <Text style={styles.value}>{formatDate(financement.dateDeFinancement)}</Text>
                            </View>
                            <View style={styles.infoRow}>
                                <Text style={styles.label}>Paiement:</Text>
                                <Text style={styles.value}>{financement.methodeDePaiement}</Text>
                            </View>

                            <View style={styles.infoRow}>
                                <Text style={styles.label}>Statut:</Text>
                                <AppIcon
                                    name={financement.statutFinancement === "Approved" ? "bookmark-check" :
                                        financement.statutFinancement === "Pending" ? "bookmark" : "bookmark-remove"}
                                    type="MaterialCommunityIcons"
                                    size={40}
                                    color={financement.statutFinancement === "Approved" ? "green" :
                                        financement.statutFinancement === "Pending" ? "orange" : "red"}
                                />
                            </View>
                        </View>
                    ))
